...
def main():
    ...
    for each in api.circuitdb_circuits():
        if (each['circuit_type'] == 'TELCO' or each['circuit_type'] == 'INTRA_DC') and each['status'] == 'production':
            TID.append(each['tid'])
    for each_tid in TID:
        uri = "https://portal.tower-research.com/portal/api/v1/netdb/tid/?username=netops-api&api_key=xxxxxxx&alias__contains=TID%s&is_active=true&device__status=1" % (each_tid)
